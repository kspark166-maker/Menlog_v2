...
const DEMO_SHOPS = [
  { name:"らぁ麺 飯田商店",  score:98.5, genre:"塩",   area:"湯河原", id:"30132"  },
  { name:"中華蕎麦 とみ田",  score:97.2, genre:"つけ麺",area:"松戸",  id:"282"    },
  { name:"Japanese Soba Noodles 蔦", score:96.0, genre:"醤油", area:"代々木上原", id:"121857" },
  { name:"創始麺屋武蔵",     score:88.5, genre:"醤油",  area:"新宿",  id:"83"     },
  { name:"風雲児",           score:89.3, genre:"鶏白湯",area:"代々木",id:"12119"  },
];
...
async function analyzeRamenWithAI(imageBase64, mediaType, extraContext) {
  ...
  let locationHint = "";
  if (gps) locationHint += `EXIF GPS座標: 緯度${gps.lat.toFixed(5)}, 経度${gps.lon.toFixed(5)}。`;
  if (browserLocation) locationHint += ` ブラウザ位置情報: ${browserLocation}。`;
  if (fileName) locationHint += ` ファイル名: ${fileName}。`;
  if (exifDate) locationHint += ` 撮影日: ${exifDate}。`;

  const systemPrompt = `あなたはラーメン画像分析の専門家です。ユーザーが送る画像を分析し、以下を特定してください。

## 分析手順
1. 画像からラーメンの特徴を観察（スープの色・濁度、麺の太さ、具材、器の特徴、店内の雰囲気、看板やメニュー等のテキスト）
2. GPS座標や位置情報がある場合、その周辺のラーメン店を推理
3. web_searchツールで「ramendb.supleks.jp」を検索して該当店舗を特定
4. 検索クエリ例: 「site:ramendb.supleks.jp [エリア名] [ジャンル]」「ramendb.supleks.jp [店舗名]」

## 必ずJSON形式のみで回答
以下のJSON形式のみを返してください。マークダウンのバッククォートなし、説明文なし：
{
  "shopName": "店舗名（特定できない場合null）",
  "genre": "ジャンル（醤油/豚骨/塩/味噌/つけ麺/鶏白湯/二郎系/中華そば/煮干し/担々麺/その他）",
  "area": "エリア（特定できない場合空文字）",
  "menu": "メニュー名の推測（不明なら空文字）",
  "ramendbId": "ラーメンDBのIDまたはURL（見つからなければnull）",
  "ramendbUrl": "ラーメンDBの店舗ページURL（見つからなければnull）",
  "confidence": 0-100の数値,
  "reasoning": "判定理由の短い説明"
}`;
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: "text", text: `この画像のラーメンを分析してください。\n${locationHint}\nラーメンデータベース(ramendb.supleks.jp)で該当する店舗を検索して特定してください。` }
          ]
        }]
      })
    });
...
async function runAIBulk({ files, entries, location, onProgress }) {
  ...
  for (let i=0; i<files.length; i++) {
    ...
    onProgress(i+1, files.length, "🤖 AI分析中...");
    await new Promise(r => setTimeout(r, 10));

    // Resize image for API (max ~800px to save tokens)
    ...
    if (aiResult && aiResult.shopName && aiResult.confidence >= 30) {
    ...
    if (existing) {
      ...
      work.push({
        ...
        comment: known
          ? `AI振分: ${shopName}（信頼度${confidence}%）${aiResult?.reasoning ? "\n" + aiResult.reasoning : ""}`
          : `AI判定: 店舗不明・推定ジャンル[${genre}]${aiResult?.reasoning ? "\n" + aiResult.reasoning : ""}`,
        ramendbId,
        ramendbUrl,
        aiDetected: true,
      });
...
function FullscreenSpinner({ shopName, progress, total }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:9999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32}}>
      ...
      <div style={{color:"#fff",fontWeight:700,fontSize:20,marginBottom:6,fontFamily:"'Shippori Mincho',Georgia,serif"}}>AI画像認識中...</div>
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:t.bg, position:"relative" }}>
      ...
      {aiLoading && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.9)", zIndex:50, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32 }}>
          ...
          <div style={{ color:"white", fontWeight:700, fontSize:20, marginBottom:6 }}>AI画像認識中...</div>
          <div style={{ color:"rgba(255,255,255,0.6)", fontSize:12, marginBottom:10 }}>Claude APIでラーメンDBを検索して店舗を特定</div>
        ...
        {entries.length === 0 ? (
          <div style={{ textAlign:"center", padding:"52px 20px", color:t.txm }}>
            ...
            <div style={{ fontSize:12, marginBottom:20, lineHeight:1.7 }}>
              「🤖 AI取込」から複数枚選択すると<br/>Claude APIでラーメンDBを検索して<br/>自動でアルバムを作成します
            </div>
            <button
      ...
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
        <div style={{ background:t.card, width:"100%", borderRadius:"20px 20px 0 0", padding:"0 20px 32px", maxHeight:"80vh", overflowY:"auto" }}>
          ...
          <div style={{ textAlign:"center", marginBottom:16 }}>
            ...
            <div style={{ fontSize:12, color:t.txm, marginTop:4 }}>Claude APIでラーメンDBを検索して自動振り分け</div>
          </div>
...
function AiBulkTrigger({ onDone, onEntriesUpdate }) {
  ...
  return (
    <div onClick={()=>bulkRef.current?.click()} style={{display:"block",padding:"16px 14px",background:t.grad,borderRadius:14,textAlign:"center",cursor:"pointer"}}>
      ...
      <div style={{color:"white",fontWeight:700,fontSize:14,marginBottom:2}}>画像を一括取込（AI画像認識）</div>
      <div style={{color:"rgba(255,255,255,0.8)",fontSize:11}}>複数枚選択可 · Claude API + ラーメンDB検索 · GPS位置推論</div>
