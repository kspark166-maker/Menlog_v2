...
## web_searchで店舗を検索
テキストが読み取れた場合、ramendb.supleks.jpで店舗を検索してください。
...
{"shopName":"店舗名 or null","menuName":"推定メニュー名","genre":"醤油/豚骨/塩/味噌/つけ麺/鶏白湯/二郎系/家系/煮干し/担々麺/中華そば/その他","soupType":"スープ詳細","noodleType":"麺の詳細","toppings":["具材1","具材2"],"area":"エリア","estimatedPrice":"推定価格","ramendbId":"数字ID or null","ramendbUrl":"URL or null","confidence":0-100,"detectedTexts":["読取テキスト"],"reasoning":"判定理由50字以内"}`;
  ...
  const resp = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: [
        ...
        { type: "text", text: `このラーメン画像を解析してください。画像内のテキスト（ロゴ・看板・箸袋等）を最優先で読み取り、ramendb.supleks.jpで店舗を検索してください。最後に必ずJSON形式で結果を出力してください。${contextBlock}` }
      ]}]
    })
  });
  if (!resp.ok) {
  ...
  console.log("[AI v7] Phase1 stop_reason:", data.stop_reason, "content_blocks:", data.content?.length);
  return data;
}

// ── Phase 2: ビジュアルのみ解析 (web_search無し・高速) ──
async function aiPhase2_visualOnly(base64Data, mt, contextBlock) {
  const sys = `ラーメン画像を分析してください。画像内のテキスト（どんぶりのロゴ、箸袋の文字、看板等）を必ず読み取ってください。

回答は純粋なJSONのみ（バッククォート禁止・説明文禁止）:
{"shopName":"読み取れた店舗名 or null","menuName":"推定メニュー","genre":"醤油/豚骨/塩/味噌/つけ麺/鶏白湯/二郎系/家系/煮干し/担々麺/中華そば/その他","soupType":"スープ色と特徴","noodleType":"麺の特徴","toppings":["具材"],"area":"","estimatedPrice":"","ramendbId":null,"ramendbUrl":null,"confidence":0-100,"detectedTexts":["画像内で読めたテキスト全て"],"reasoning":"特徴の説明"}`;
  ...
  const resp = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: [
        { type: "image", source: { type: "base64", media_type: mt, data: base64Data } },
        { type: "text", text: `このラーメン画像を分析してください。最も重要: どんぶりの縁や内側、箸袋、背景の看板などに書かれた文字を全て読み取ってください。次に、スープ色・麺の太さ・具材からジャンルと特徴を判定してください。${contextBlock}` }
      ]}]
    })
  });
  if (!resp.ok) {
  ...
  const data = await resp.json();
  console.log("[AI v7] Phase2 stop_reason:", data.stop_reason, "content_blocks:", data.content?.length);
  return data;
}
...
async function aiPhase3_minimalGenre(base64Data, mt) {
  const resp = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: [
        { type: "image", source: { type: "base64", media_type: mt, data: base64Data } },
        { type: "text", text: `この画像を見て答えてください。どんぶりや箸袋に店名が書いてありますか？あればその文字を教えてください。また、ラーメンのジャンルを判定してください。回答はJSONのみ: {"shopName":"読めた店名 or null","genre":"醤油/豚骨/塩/味噌/つけ麺/鶏白湯/二郎系/家系/煮干し/その他","soupType":"スープの色","menuName":"推定メニュー名","detectedTexts":["読めたテキスト"],"confidence":0-100,"reasoning":"理由20字"}` }
      ]}]
    })
  });
  if (!resp.ok) {
  ...
  const data = await resp.json();
  console.log("[AI v7] Phase3 stop_reason:", data.stop_reason);
  return data;
}

// ── APIレスポンスからJSONを抽出（全block型対応） ──
function extractJsonFromResponse(data) {
  if (!data || !data.content) {
    console.warn("[AI v7] extractJson: data.content が空です");
    return null;
  }

  // 全てのtextブロックからテキストを収集
  const texts = [];
  ...
  console.log("[AI v7] extractJson: テキストブロック数:", texts.length, "合計文字数:", allText.length, "先頭200字:", allText.slice(0, 200));

  if (!allText.trim()) {
    // textブロックがない場合 → content全体をログ出力
    console.warn("[AI v7] テキストブロックなし。全blockのtype:", data.content.map(b => b.type).join(","));
    return null;
  }
  ...
  // 方法1: 最後のJSON objectを探す（web_search結果の後に出力されるJSON）
  const allMatches = [...cleaned.matchAll(/\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/g)];
  if (allMatches.length > 0) {
    // 最後のマッチが結果JSONである可能性が高い（web_search結果の後）
    for (let i = allMatches.length - 1; i >= 0; i--) {
      try {
        const parsed = JSON.parse(allMatches[i][0]);
        // shopName or genre フィールドがあればこれが正解
        if ("genre" in parsed || "shopName" in parsed) {
          // Normalize IDs
          ...
          console.log("[AI v7] extractJson 成功: shopName=", parsed.shopName, "genre=", parsed.genre, "confidence=", parsed.confidence);
          return parsed;
        }
      } catch {}
    }
  ...
  // 方法2: より緩い正規表現
  const looseMatch = cleaned.match(/\{[\s\S]*?"(?:shopName|genre)"[\s\S]*?\}/);
  if (looseMatch) {
    try {
      const parsed = JSON.parse(looseMatch[0]);
      if (parsed.menuName && !parsed.menu) parsed.menu = parsed.menuName;
      console.log("[AI v7] extractJson (loose) 成功:", parsed.shopName || parsed.genre);
      return parsed;
    } catch (e) {
      console.warn("[AI v7] extractJson loose parse失敗:", e.message);
    }
  }

  console.warn("[AI v7] extractJson: JSON抽出失敗。テキスト先頭300字:", cleaned.slice(0, 300));
  return null;
}
...
async function analyzeRamenWithAI(imageBase64, mediaType, extraContext) {
  ...
  // Phase 1: フル解析 (web_search付き)
  try {
    console.log("[AI v7] Phase1: フル解析開始... base64 size:", base64Data.length, "bytes");
    ...
    if (result && (result.shopName || result.genre)) {
      console.log("[AI v7] Phase1 成功:", JSON.stringify({shopName:result.shopName, genre:result.genre, confidence:result.confidence, detectedTexts:result.detectedTexts}));
      return result;
    }
    console.warn("[AI v7] Phase1: JSON抽出失敗またはshopName/genreなし。Phase2へ");
  } catch (e) {
  ...
  try {
    ...
    const result = extractJsonFromResponse(data);
    if (result && (result.genre || result.shopName)) {
      console.log("[AI v7] Phase2 成功:", JSON.stringify({shopName:result.shopName, genre:result.genre, detectedTexts:result.detectedTexts}));
      return result;
    }
  ...
  try {
    ...
    const result = extractJsonFromResponse(data);
    if (result) {
      console.log("[AI v7] Phase3 成功:", JSON.stringify({shopName:result.shopName, genre:result.genre, detectedTexts:result.detectedTexts}));
      return result;
    }
    console.warn("[AI v7] Phase3: JSON抽出失敗");
  } catch (e) {
