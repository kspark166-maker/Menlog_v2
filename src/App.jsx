...
function Provider({ children }) {
  ...
  const [detailEntryId, setDetailEntryId] = useState(null); // AlbumDetail用（transform外でレンダリング）
...
// ─── アルバム詳細（全要素を1画面に収める） ───────────────────────────────────────
// ─── アルバム詳細（Context直接使用・transform外でレンダリング） ─────────────────
            ...
            {!collapsed && (
              <div style={{ display:"flex", flexDirection:"column", gap:2, marginTop:3 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:4, fontSize:14, lineHeight:1.55 }}>
                  ...
                  <span style={{ fontSize:11, color:t.txm }}>（☆で判定）</span>
                </div>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0, padding:"0 12px 8px" }}>
            <div style={{ flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
              ...
              <input ref={addFileRef} type="file" multiple accept="image/*" style={{display:"none"}}
                onChange={ev=>{ const f=ev.target.files; ev.target.value=""; onAddImgs(entry.id,f); }}/>
            </div>
            ...
            {images.length > 0 ? (
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:t.bg2, border:`1.5px dashed ${t.br}`, borderRadius:12, color:t.txm }}>
            ...
            <button onClick={()=>{if(window.confirm(`「${entry.shopName}」を削除しますか？`)){onDelete(entry.id);}}}
              style={{ flexShrink:0, width:"100%", padding:"9px", borderRadius:9, border:"1.5px solid #FADBD8", background:"#FFF5F5", color:"#E74C3C", fontWeight:600, fontSize:12, cursor:"pointer", marginTop:7 }}>
              🗑️ このアルバムを削除
            </button>
          </div>
        </div>
...
// ─── マイページ（フレンド・グループ共有含む） ─────────
function MyPage() {
  const {entries,setEntries,groups,setGroups,profile,setProfile,settings,setSettings,friends,addFriend,removeFriend,filterMode,setFilterMode,myCode,setDetailEntryId,t}=useApp();
  ...
  // ── 日記詳細（長押し）→ Context経由でtransform外にレンダリング ──
...
function MainLayout() {
  ...
  return (
    <div style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",fontFamily:"'Noto Sans JP',ui-sans-serif,sans-serif",background:t.bg,overflow:"hidden",position:"relative"}}>
      ...
      {/* ══ AlbumDetail: スワイプコンテナ外・transform影響なし ══ */}
