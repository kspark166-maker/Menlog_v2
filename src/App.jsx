...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Claude API ラーメン画像認識エンジン v6
// (Flutter/Gemini仕様書ベースのPrompt Engineering全面改修)
//
// 分析レイヤー:
//   L1: 視覚特徴の網羅的抽出（どんぶりロゴ,箸袋,卓上,看板,メニュー表）
//   L2: スープ・麺・具材の詳細分類
//   L3: 店舗空間・内装の推理
//   L4: GPS/位置情報とのクロスリファレンス
//   L5: ラーメンDB(ramendb.supleks.jp)照合
//   L6: 価格推定
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async function analyzeRamenWithAI(imageBase64, mediaType, extraContext) {
  ...
  // ── コンテキスト情報の構築 ──
  const contextLines = [];
  if (gps) contextLines.push(`📍 EXIF GPS座標: 緯度${gps.lat.toFixed(6)}, 経度${gps.lon.toFixed(6)}`);
  if (browserLocation) contextLines.push(`📍 ブラウザ位置情報: ${browserLocation}`);
  if (fileName) contextLines.push(`📁 ファイル名: ${fileName}`);
  if (exifDate) contextLines.push(`📅 撮影日: ${exifDate}`);
  const contextBlock = contextLines.length > 0
    ? `\n\n## 付加コンテキスト情報\n${contextLines.join("\n")}`
    : "";

  const systemPrompt = `あなたはラーメン画像解析の世界的エキスパートです。
日本全国5万店以上のラーメン店の知識を持ち、どんぶりのデザイン、箸袋のロゴ、
卓上調味料の配置、店舗内装の特徴から店舗を特定できます。

## 解析プロトコル（6層分析）

### L1: テキスト・ロゴ検出（最優先・最高信頼度）
以下を画像内から徹底的に探索してください：
- **どんぶりの縁・底面・側面**に印字された店名ロゴ、紋章、文字
- **箸袋・箸の持ち手**に印刷された店名
- **卓上POP・メニュー表・壁掛けメニュー**に記載された店名・商品名・価格
- **レシート・食券**が写り込んでいる場合、その文字情報
- **暖簾・看板・ネオンサイン**が背景に写っている場合
- **紙エプロン・おしぼり袋**のブランディング
- **テーブル番号札・呼び出しベル**の特徴
→ テキストが読み取れた場合、信頼度を大幅に引き上げること

### L2: スープ・麺・具材の詳細分析
- **スープ**: 色（透明度/濁度）、表面の油膜の種類（鶏油・背脂・マー油・ラー油）、
  泡立ちの有無、粘度の推定
- **麺**: 太さ（極細〜極太）、縮れ具合、加水率の推定（低加水パツパツ系/多加水モチモチ系）、
  色味（全粒粉・卵麺等）
- **具材**: チャーシューの種類（バラ/ロース/肩ロース、煮豚/低温調理/炙り）、
  味玉の断面（半熟度）、メンマ、海苔の枚数、ネギの種類（青/白/九条）、
  ナルト、ワンタン、ほうれん草等
- **特殊トッピング**: トリュフ、燻製系、チーズ、バター等

### L3: 提供スタイル・器の分析
- どんぶりの形状（丸/変形）、素材（陶器/メラミン）、色・柄
- つけ麺の場合：麺皿とつけ汁器の構成
- まぜそば/油そば/汁なし系の判定
- 盛り付けスタイル（二郎系の山盛り/上品な懐石風/家系の海苔3枚立て等）

### L4: 店舗空間の推理
- カウンター/テーブル/座敷の判別
- 木目基調/無骨系/モダン/フードコートの区別
- 製麺機が見える場合→自家製麺
- 背景に他の客やスタッフが見える場合の雰囲気

### L5: GPS・位置情報クロスリファレンス
GPS座標がある場合、その半径2km圏内のラーメン店を推理し、
ビジュアル特徴と照合してください。

### L6: ラーメンDB照合（web_search使用）
上記分析で得られた手がかりを元に、web_searchツールで
「ramendb.supleks.jp」を検索してください。

検索戦略（優先順）:
1. 店名が判読できた場合: 「site:ramendb.supleks.jp [店名]」
2. エリア＋ジャンルが推定できた場合: 「ramendb.supleks.jp [エリア] [ジャンル] ラーメン」
3. 特徴的なメニュー名がある場合: 「ramendb.supleks.jp [メニュー名]」
4. GPS付近の場合: 「ramendb.supleks.jp [最寄り駅名] ラーメン」

**重要**: 検索結果からURLに含まれる店舗ID（/s/XXXXX.html のXXXXX部分）を必ず抽出すること。

## 出力形式
必ず以下の純粋なJSON形式のみで回答してください。
マークダウン記法（バッククォート等）は絶対に使わないでください。
説明文や前置きも不要です。JSONオブジェクトのみ出力してください。

{
  "shopName": "店舗名（特定できない場合null）",
  "menuName": "注文メニュー名の推測（例：特製醤油らーめん）",
  "genre": "醤油/豚骨/塩/味噌/つけ麺/鶏白湯/二郎系/家系/中華そば/煮干し/担々麺/まぜそば/その他",
  "soupType": "スープの詳細説明（例：清湯醤油・鶏油浮き）",
  "noodleType": "麺の説明（例：中細ストレート低加水）",
  "toppings": ["チャーシュー","味玉","海苔","メンマ"],
  "area": "エリア/最寄り駅（特定できない場合空文字）",
  "estimatedPrice": "推定価格（例：1000円、不明なら空文字）",
  "ramendbId": "ラーメンDBの店舗ID（数字のみ。見つからなければnull）",
  "ramendbUrl": "ラーメンDBの完全URL（見つからなければnull）",
  "confidence": 0から100の数値,
  "detectedTexts": ["画像内で読み取れたテキストの配列"],
  "reasoning": "判定理由（どの証拠から特定したかを具体的に。100文字以内）",
  "visualFeatures": {
    "soupColor": "スープの色（例：琥珀色、乳白色、漆黒）",
    "soupClarity": "clear/slightly_cloudy/cloudy/opaque",
    "oilType": "鶏油/背脂/マー油/ラー油/なし",
    "bowlStyle": "丸型白/丸型黒/変形/二郎系デカ盛り/つけ麺セット"
  }
}`;
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: "text", text: `このラーメン画像を6層分析プロトコルで解析してください。

画像内のテキスト（どんぶりロゴ、箸袋、看板、メニュー表、食券等）を最優先で検出し、
スープ・麺・具材の特徴を詳細に分類した上で、
ラーメンデータベース(ramendb.supleks.jp)をweb_searchで検索して店舗を特定してください。

検索結果のURLから店舗ID（/s/数字.htmlの数字部分）を必ず抽出してください。${contextBlock}` }
          ]
        }]
      })
    });
    ...
    if (jsonMatch) {
      try {
        const parsed = JSON.parse(jsonMatch[0]);
        // Normalize ramendbId: extract numeric ID if URL is given
        if (!parsed.ramendbId && parsed.ramendbUrl) {
          const idMatch = parsed.ramendbUrl.match(/\/s\/(\d+)/);
          if (idMatch) parsed.ramendbId = idMatch[1];
        }
        if (parsed.ramendbId && !parsed.ramendbUrl) {
          parsed.ramendbUrl = `https://ramendb.supleks.jp/s/${parsed.ramendbId}.html`;
        }
        // Map menuName → menu for backward compat
        if (parsed.menuName && !parsed.menu) parsed.menu = parsed.menuName;
        return parsed;
      } catch (parseErr) {
        console.error("JSON parse error:", parseErr, jsonMatch[0].slice(0, 200));
        return null;
      }
    }
    console.warn("No JSON found in AI response:", fullText.slice(0, 300));
    return null;
  } catch (err) {
...
async function runAIBulk({ files, entries, location, onProgress }) {
  ...
  const sessionMap = {}; // date-based session grouping

  for (let i=0; i<files.length; i++) {
    const file = files[i];
    onProgress(i+1, files.length, "画像を読み込み中...");
    ...
    onProgress(i+1, files.length, "🤖 AI解析中...");
    await new Promise(r => setTimeout(r, 10));

    // Resize image for API (max 1024x1024 - Gemini仕様準拠)
    ...
    let detectedTexts = [], visualFeatures = null;

    if (aiResult && aiResult.shopName && aiResult.confidence >= 25) {
      ...
      visualFeatures = aiResult.visualFeatures || null;
      known = true;
      onProgress(i+1, files.length, `✅ ${shopName}`);
    } else {
      // Unknown - group by session (date)
      const sessionKey = visitDate.replace(/-/g,"");
      if (!sessionMap[sessionKey]) {
        unknownCounter++;
        sessionMap[sessionKey] = `不明${unknownCounter}`;
      }
      shopName = sessionMap[sessionKey];
      genre = aiResult?.genre || "その他";
      area = aiResult?.area || "";
      menu = aiResult?.menu || aiResult?.menuName || "";
      soupType = aiResult?.soupType || "";
    ...
    // Build detailed AI comment
    const commentParts = [];
    if (known) {
      commentParts.push(`AI振分: ${shopName}（信頼度${confidence}%）`);
    } else {
      commentParts.push(`AI判定: 店舗不明・推定ジャンル[${genre}]`);
    }
    if (aiResult?.reasoning) commentParts.push(aiResult.reasoning);
    ...
    if (existing) {
      ...
      work.push({
        ...
        aiDetected: true,
        aiDetails: {
          soupType, noodleType, toppings, detectedTexts, visualFeatures, confidence,
        },
      });
...
function FullscreenSpinner({ shopName, progress, total }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:9999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32}}>
      ...
      <div style={{color:"#fff",fontWeight:700,fontSize:20,marginBottom:6,fontFamily:"'Shippori Mincho',Georgia,serif"}}>AI 6層解析中...</div>
      ...
      <div style={{marginTop:14,color:"rgba(255,255,255,0.35)",fontSize:11,textAlign:"center",lineHeight:1.9}}>
        L1:テキスト・ロゴ検出 → L2:スープ・麺分析<br/>
        L3:器・盛付 → L4:GPS照合 → L5:ラーメンDB検索
      </div>
    </div>
  );
}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:t.bg, position:"relative" }}>
      ...
      {aiLoading && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.9)", zIndex:50, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32 }}>
          ...
          <div style={{ color:"white", fontWeight:700, fontSize:20, marginBottom:6 }}>AI 6層解析中...</div>
          <div style={{ color:"rgba(255,255,255,0.6)", fontSize:12, marginBottom:10 }}>テキスト検出 → スープ・麺分析 → ラーメンDB照合</div>
          ...
          <div style={{ marginTop:18, color:"rgba(255,255,255,0.28)", fontSize:11, textAlign:"center", lineHeight:1.9 }}>
            どんぶりロゴ・箸袋・看板のテキスト検出<br/>
            スープ色・麺質・具材の詳細分類 + GPS推論
          </div>
        </div>
      )}
      ...
      <div style={{ flex:1, overflowY:"auto", padding:10 }}>
        {entries.length === 0 ? (
          <div style={{ textAlign:"center", padding:"52px 20px", color:t.txm }}>
            ...
            <div style={{ fontSize:12, marginBottom:20, lineHeight:1.7 }}>
              「🤖 AI取込」から複数枚選択すると<br/>6層解析（ロゴ検出→スープ分析→ラーメンDB照合）で<br/>自動でアルバムを作成します
            </div>
            <button
      ...
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
        <div style={{ background:t.card, width:"100%", borderRadius:"20px 20px 0 0", padding:"0 20px 32px", maxHeight:"80vh", overflowY:"auto" }}>
          ...
          <div style={{ textAlign:"center", marginBottom:16 }}>
            ...
            <div style={{ fontSize:12, color:t.txm, marginTop:4 }}>6層解析でラーメンDBを検索して自動振り分け</div>
          </div>
...
function AiBulkTrigger({ onDone, onEntriesUpdate }) {
  ...
  return (
    <div onClick={()=>bulkRef.current?.click()} style={{display:"block",padding:"16px 14px",background:t.grad,borderRadius:14,textAlign:"center",cursor:"pointer"}}>
      ...
      <div style={{color:"white",fontWeight:700,fontSize:14,marginBottom:2}}>画像を一括取込（AI 6層解析）</div>
      <div style={{color:"rgba(255,255,255,0.8)",fontSize:11}}>複数枚選択可 · ロゴ/テキスト検出 · スープ分析 · ラーメンDB照合</div>
